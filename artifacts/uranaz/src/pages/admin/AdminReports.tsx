...
function StatCard({ label, value, sub, color = TEAL }: any) {
  return (
    <div className="rounded-xl p-3 sm:p-4" style={GLASS}>
      <div className="text-[11px] uppercase tracking-wider" style={{ color: "rgba(168,237,255,0.40)", letterSpacing: "0.08em" }}>
        {label}
      </div>
...
function Pill({ label, color, bg }: { label: string; color: string; bg: string }) {
  return (
    <span
      className="text-[10px] sm:text-[11px] font-semibold px-2 py-0.5 rounded-full capitalize whitespace-nowrap"
...
export default function AdminReports() {
  ...
  const handleExport = async () => {
    ...
    // Pull the WHOLE filtered set (not just current page) for CSV.
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex-1 min-w-[180px] relative">
            ...
            <input
              ...
              placeholder={tab === "wallet-changes" ? "Search user, email, address…" : "Search user, email, tx hash…"}
          ...
          {tab === "wallet-changes" && (
            <label className="flex items-center gap-2 text-xs cursor-pointer" style={{ color: "rgba(168,237,255,0.65)" }}>
              <input
                type="checkbox"
                checked={otpOnly}
                onChange={e => { setOtpOnly(e.target.checked); setPage(1); }}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(from || to) && (
            <button
              ...
              style={{ color: "rgba(168,237,255,0.45)" }}
            >
              Clear dates
            </button>
          )}
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead>
                <tr style={{ background: "rgba(61,214,245,0.05)", color: "rgba(168,237,255,0.55)" }}>
                  {tab === "deposits" && (
                    <>
                      <Th>User</Th><Th>Amount</Th><Th>Status</Th><Th className="hidden sm:table-cell">TX Hash</Th><Th>Created</Th>
                    </>
                  )}
                  {tab === "withdrawals" && (
                    <>
                      <Th>User</Th><Th>Amount</Th><Th>Wallet</Th><Th>Status</Th><Th className="hidden sm:table-cell">TX Hash</Th><Th>Requested</Th>
                    </>
                  )}
                  {tab === "wallet-changes" && (
                    <>
                    ...
                    <Tr key={r.id} testId={`row-walletchange-${r.id}`}>
                      ...
                      <Td>
                        <div className="font-mono text-[11px] leading-snug" style={{ color: "rgba(168,237,255,0.65)" }}>
                          {r.isInitialSetup
                            ? <span style={{ color: "rgba(168,237,255,0.4)" }}>(initial setup)</span>
                            : <span>{shortAddr(r.oldAddress)}</span>}
                          <div style={{ color: TEAL }}>→ {shortAddr(r.newAddress)}</div>
                        </div>
                      </Td>
                      <Td>
                        {r.isInitialSetup
                          ? <Pill label="initial" color="rgba(168,237,255,0.55)" bg="rgba(168,237,255,0.05)" />
                          : <Pill label="updated" color="#fbbf24" bg="rgba(251,191,36,0.10)" />}
                      </Td>
                      <Td>
                        {r.otpVerified
                          ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-semibold" style={{ color: "#34d399" }}>
                      ...
                      <Td className="hidden sm:table-cell">
                        <span className="font-mono text-[11px]" style={{ color: "rgba(168,237,255,0.5)" }}>{r.ipAddress || "—"}</span>
                      </Td>
                      <Td><span style={{ color: "rgba(168,237,255,0.55)" }}>{fmtDate(r.createdAt)}</span></Td>
                    </Tr>
                  );
...
function Td({ children, className = "" }: any) {
  return <td className={`px-3 py-2.5 align-top whitespace-nowrap ${className}`}>{children}</td>;
}
function Tr({ children, testId }: any) {
  return (
    <tr data-testid={testId} className="transition-colors" style={{ borderTop: "1px solid rgba(61,214,245,0.05)" }}>
      {children}
    </tr>
  );
}
function UserCell({ name, email, id }: { name: string; email: string; id: number }) {
  return (
    <div className="leading-tight">
      <div className="font-semibold text-xs" style={{ color: "rgba(168,237,255,0.85)" }}>{name}</div>
      <div className="text-[11px]" style={{ color: "rgba(168,237,255,0.4)" }}>{email}</div>
      <div className="text-[10px]" style={{ color: "rgba(168,237,255,0.25)" }}>#{id}</div>
    </div>
  );
}
function TxLink({ hash }: { hash: string }) {
  return (
    <a
      ...
      style={{ color: TEAL }}
    >
      <ExternalLink size={10} /> {hash.slice(0, 10)}…{hash.slice(-6)}
    </a>
  );
}
